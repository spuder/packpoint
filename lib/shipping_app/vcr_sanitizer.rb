    ...
    class << self
      ...
      def sanitize_response(response)
        ...
        begin
          ...
          if data.is_a?(Array) && data.all? { |item| item.is_a?(Hash) && item.key?('json_parsed') }
            sanitize_tindie_orders(data)
          elsif data['orders']
            sanitize_orders(data['orders'])
          end
      ...
      def sanitize_tindie_orders(orders)
        orders.each do |order|
          sanitize_order_fields(order['json_parsed'])
        end
      end
